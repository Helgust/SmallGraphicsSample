...
void MemoryService::init(void* configuration) {
  ...
  system_allocator.init(memory_configuration
                            ? memory_configuration->maximum_dynamic_size
                            : s_size);
}
...
void* HeapAllocator::allocate(sizet size, sizet alignment, cstring file,
                              i32 line) {
...
void* LinearAllocator::allocate(sizet size, sizet alignment, cstring file,
                                i32 line) {
  return allocate(size, alignment);
}
...
void* MallocAllocator::allocate(sizet size, sizet alignment, cstring file,
                                i32 line) {
  return malloc(size);
}
...
void* StackAllocator::allocate(sizet size, sizet alignment, cstring file,
                               i32 line) {
  return allocate(size, alignment);
}
...
void StackAllocator::deallocate(void* pointer) {
  ...
  RASSERTM(pointer < memory + total_size,
           "Out of bound free on linear allocator (outside bounds). Tempting "
           "to free %p, %llu after beginning of buffer (memory %p size %llu, "
           "allocated %llu)",
           (u8*)pointer, (u8*)pointer - memory, memory, total_size,
           allocated_size);
  RASSERTM(pointer < memory + allocated_size,
           "Out of bound free on linear allocator (inside bounds, after "
           "allocated). Tempting to free %p, %llu after beginning of buffer "
           "(memory %p size %llu, allocated %llu)",
           (u8*)pointer, (u8*)pointer - memory, memory, total_size,
           allocated_size);
...
void* DoubleStackAllocator::allocate(sizet size, sizet alignment, cstring file,
                                     i32 line) {
  RASSERT(false);
  return nullptr;
}
