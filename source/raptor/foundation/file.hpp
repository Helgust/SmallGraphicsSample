...
void file_directory_from_path(
    char*
        path);  // Retrieve path without the filename. Path is a preallocated string buffer. It moves the terminator before the name of the file.
...
void file_find_files_in_path(
    ...
    StringArray&
        files);  // Search files matching file_pattern and puts them in files array.
                 // Examples: "..\\data\\*", "*.bin", "*.*"
void file_find_files_in_path(
    cstring extension, cstring search_pattern, StringArray& files,
    StringArray&
        directories);  // Search files and directories using search_patterns.
