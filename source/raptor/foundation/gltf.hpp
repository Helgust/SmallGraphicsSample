...
#define InjectDefault3DModel()                                                 \
  if (raptor::file_exists(kDefault3DModel)) {                                  \
    ...
    printf(                                                                    \
        "Unable to find default model. Please check the README in the root "   \
        "folder and make sure you've run `python ./bootstrap.py` to download " \
        "all the additional assets for this project.\n");                      \
    exit(-1);                                                                  \
  }
