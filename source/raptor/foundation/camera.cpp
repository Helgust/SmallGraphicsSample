...
void Camera::init_perpective(f32 near_plane_, f32 far_plane_, f32 fov_y,
                             f32 aspect_ratio_) {
  perspective = true;
...
void Camera::init_orthographic(f32 near_plane_, f32 far_plane_,
                               f32 viewport_width_, f32 viewport_height_,
                               f32 zoom_) {
  perspective = false;
...
void Camera::update() {
  ...
  const versors rotation =
      glms_quat_normalize(glms_quat_mul(pitch_rotation, yaw_rotation));

  const mat4s translation =
      glms_translate_make(glms_vec3_scale(position, -1.f));
...
void Camera::calculate_projection_matrix() {
  if (perspective) {
    projection = glms_perspective(glm_rad(field_of_view_y), aspect_ratio,
                                  near_plane, far_plane);
  } else {
    projection =
        glms_ortho(zoom * -viewport_width / 2.f, zoom * viewport_width / 2.f,
                   zoom * -viewport_height / 2.f, zoom * viewport_height / 2.f,
                   near_plane, far_plane);
  }
}
...
vec3s Camera::unproject_inverted_y(const vec3s& screen_coordinates) {
  const vec3s screen_coordinates_y_inv{screen_coordinates.x,
                                       viewport_height - screen_coordinates.y,
                                       screen_coordinates.z};
  return unproject(screen_coordinates_y_inv);
}

void Camera::get_projection_ortho_2d(mat4& out_matrix) {
  glm_ortho(0, viewport_width * zoom, 0, viewport_height * zoom, -1.f, 1.f,
            out_matrix);
}

void Camera::yaw_pitch_from_direction(const vec3s& direction, f32& yaw,
                                      f32& pitch) {
