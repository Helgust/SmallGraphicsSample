...
void StringView::copy_to(const StringView& a, char* buffer, sizet buffer_size) {
  ...
  const sizet max_length =
      buffer_size - 1 < a.length ? buffer_size - 1 : a.length;
...
void StringBuffer::append(const StringView& text) {
  const sizet max_length = current_size + text.length < buffer_size
                               ? text.length
...
char* StringBuffer::append_use_substring(const char* string, u32 start_index,
                                         u32 end_index) {
...
void StringArray::init(u32 size, Allocator* allocator_) {
  ...
  strings_iterator =
      (FlatHashMapIterator*)(allocated_memory + sizeof(FlatHashMap<u64, u32>));

  data = allocated_memory + sizeof(FlatHashMap<u64, u32>) +
         sizeof(FlatHashMapIterator);
