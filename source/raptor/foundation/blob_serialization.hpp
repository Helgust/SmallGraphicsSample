...
struct BlobSerializer {
  ...
  template <typename T>
  T* write_and_prepare(Allocator* allocator, u32 serializer_version,
                       sizet size);

  template <typename T>
  void write_and_serialize(Allocator* allocator, u32 serializer_version,
                           sizet size, T* root_data);
  ...
  template <typename T>
  T* read(Allocator* allocator, u32 serializer_version, sizet size,
          char* blob_memory, bool force_serialization = false);
  ...
  char* allocate_static(
      sizet
          size);  // Just allocate size bytes and return. Used to fill in structures.
  ...
  void allocate_and_set(
      ...
      void* source_data =
          nullptr);  // Allocate an array and set it so it can be accessed.
  ...
  u32 data_version =
      0xffffffff;  // Version read from blob or written into blob.
...
template <typename T>
T* BlobSerializer::write_and_prepare(Allocator* allocator_,
...
template <typename T>
void BlobSerializer::write_and_serialize(Allocator* allocator_,
                                         u32 serializer_version_, sizet size,
                                         T* data) {
...
template <typename T>
T* BlobSerializer::read(Allocator* allocator_, u32 serializer_version_,
                        sizet size, char* blob_memory_,
...
template <typename T>
inline void BlobSerializer::allocate_and_set(RelativePointer<T>& data,
                                             void* source_data) {
...
template <typename T>
inline void BlobSerializer::allocate_and_set(RelativeArray<T>& data,
                                             u32 num_elements,
                                             void* source_data) {
  ...
  if (source_data) {
    raptor::memory_copy(destination_memory, source_data,
...
inline void BlobSerializer::serialize(Array<T>* data) {
  ...
  if (is_reading) {
    ...
    allocate_static(data->size * sizeof(T));
    //
    serialized_offset = cached_serialized + source_data_offset -
                        sizeof(u32);  // -sizeof( u64 ) * 2;
