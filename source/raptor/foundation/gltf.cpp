...
static void try_load_string(json& json_data, cstring key,
                            StringBuffer& string_buffer, Allocator* allocator) {
...
static void try_load_type(json& json_data, cstring key,
...
static void try_load_int_array(json& json_data, cstring key, u32& count,
                               i32** array, Allocator* allocator) {
  auto it = json_data.find(key);
  if (it == json_data.end()) {
    count = 0;
    *array = nullptr;
...
static void try_load_float_array(json& json_data, cstring key, u32& count,
                                 float** array, Allocator* allocator) {
  auto it = json_data.find(key);
  if (it == json_data.end()) {
    count = 0;
    *array = nullptr;
...
static void load_asset(json& json_data, glTF::Asset& asset,
                       Allocator* allocator) {
...
static void load_scene(json& json_data, glTF::Scene& scene,
                       Allocator* allocator) {
  try_load_int_array(json_data, "nodes", scene.nodes_count, &scene.nodes,
                     allocator);
}

static void load_scenes(json& json_data, glTF::glTF& gltf_data,
                        Allocator* allocator) {
  json scenes = json_data["scenes"];

  sizet scene_count = scenes.size();
  gltf_data.scenes = (glTF::Scene*)allocate_and_zero(
      allocator, sizeof(glTF::Scene) * scene_count);
...
static void load_buffer(json& json_data, glTF::Buffer& buffer,
                        Allocator* allocator) {
...
static void load_buffers(json& json_data, glTF::glTF& gltf_data,
                         Allocator* allocator) {
  json buffers = json_data["buffers"];

  sizet buffer_count = buffers.size();
  gltf_data.buffers = (glTF::Buffer*)allocate_and_zero(
      allocator, sizeof(glTF::Buffer) * buffer_count);
...
static void load_buffer_view(json& json_data, glTF::BufferView& buffer_view,
                             Allocator* allocator) {
...
static void load_buffer_views(json& json_data, glTF::glTF& gltf_data,
                              Allocator* allocator) {
  json buffers = json_data["bufferViews"];

  sizet buffer_count = buffers.size();
  gltf_data.buffer_views = (glTF::BufferView*)allocate_and_zero(
      allocator, sizeof(glTF::BufferView) * buffer_count);
...
static void load_node(json& json_data, glTF::Node& node, Allocator* allocator) {
  ...
  try_load_int_array(json_data, "children", node.children_count, &node.children,
                     allocator);
  try_load_float_array(json_data, "matrix", node.matrix_count, &node.matrix,
                       allocator);
  try_load_float_array(json_data, "rotation", node.rotation_count,
                       &node.rotation, allocator);
  try_load_float_array(json_data, "scale", node.scale_count, &node.scale,
                       allocator);
  try_load_float_array(json_data, "translation", node.translation_count,
                       &node.translation, allocator);
  try_load_float_array(json_data, "weights", node.weights_count, &node.weights,
                       allocator);
  try_load_string(json_data, "name", node.name, allocator);
}

static void load_nodes(json& json_data, glTF::glTF& gltf_data,
                       Allocator* allocator) {
  json array = json_data["nodes"];

  sizet array_count = array.size();
  gltf_data.nodes = (glTF::Node*)allocate_and_zero(
      allocator, sizeof(glTF::Node) * array_count);
...
static void load_mesh_primitive(json& json_data,
  ...
  mesh_primitive.attributes =
      (glTF::MeshPrimitive::Attribute*)allocate_and_zero(
          allocator,
          sizeof(glTF::MeshPrimitive::Attribute) * attributes.size());
  mesh_primitive.attribute_count = attributes.size();

  u32 index = 0;
  for (auto json_attribute : attributes.items()) {
    std::string key = json_attribute.key();
    glTF::MeshPrimitive::Attribute& attribute =
        mesh_primitive.attributes[index];
...
static void load_mesh_primitives(json& json_data, glTF::Mesh& mesh,
                                 Allocator* allocator) {
  json array = json_data["primitives"];

  sizet array_count = array.size();
  mesh.primitives = (glTF::MeshPrimitive*)allocate_and_zero(
      allocator, sizeof(glTF::MeshPrimitive) * array_count);
...
static void load_mesh(json& json_data, glTF::Mesh& mesh, Allocator* allocator) {
  ...
  try_load_float_array(json_data, "weights", mesh.weights_count, &mesh.weights,
                       allocator);
  try_load_string(json_data, "name", mesh.name, allocator);
}

static void load_meshes(json& json_data, glTF::glTF& gltf_data,
                        Allocator* allocator) {
  json array = json_data["meshes"];

  sizet array_count = array.size();
  gltf_data.meshes = (glTF::Mesh*)allocate_and_zero(
      allocator, sizeof(glTF::Mesh) * array_count);
...
static void load_accessor(json& json_data, glTF::Accessor& accessor,
                          Allocator* allocator) {
  ...
  try_load_float_array(json_data, "max", accessor.max_count, &accessor.max,
                       allocator);
  try_load_float_array(json_data, "min", accessor.min_count, &accessor.min,
                       allocator);
  try_load_bool(json_data, "normalized", accessor.normalized);
  try_load_type(json_data, "type", accessor.type);
}

static void load_accessors(json& json_data, glTF::glTF& gltf_data,
                           Allocator* allocator) {
  json array = json_data["accessors"];

  sizet array_count = array.size();
  gltf_data.accessors = (glTF::Accessor*)allocate_and_zero(
      allocator, sizeof(glTF::Accessor) * array_count);
...
static void try_load_TextureInfo(json& json_data, cstring key,
  ...
  glTF::TextureInfo* ti =
      (glTF::TextureInfo*)allocator->allocate(sizeof(glTF::TextureInfo), 64);
...
static void try_load_MaterialNormalTextureInfo(
    json& json_data, cstring key,
    glTF::MaterialNormalTextureInfo** texture_info, Allocator* allocator) {
  auto it = json_data.find(key);
  if (it == json_data.end()) {
    *texture_info = nullptr;
    return;
  ...
  glTF::MaterialNormalTextureInfo* ti =
      (glTF::MaterialNormalTextureInfo*)allocator->allocate(
...
static void try_load_MaterialOcclusionTextureInfo(
    json& json_data, cstring key,
    glTF::MaterialOcclusionTextureInfo** texture_info, Allocator* allocator) {
  auto it = json_data.find(key);
  if (it == json_data.end()) {
    *texture_info = nullptr;
    return;
  ...
  glTF::MaterialOcclusionTextureInfo* ti =
      (glTF::MaterialOcclusionTextureInfo*)allocator->allocate(
...
static void try_load_MaterialPBRMetallicRoughness(
    json& json_data, cstring key,
    glTF::MaterialPBRMetallicRoughness** texture_info, Allocator* allocator) {
  auto it = json_data.find(key);
  if (it == json_data.end()) {
    *texture_info = nullptr;
    return;
  ...
  glTF::MaterialPBRMetallicRoughness* ti =
      (glTF::MaterialPBRMetallicRoughness*)allocator->allocate(
          sizeof(glTF::MaterialPBRMetallicRoughness), 64);

  try_load_float_array(*it, "baseColorFactor", ti->base_color_factor_count,
                       &ti->base_color_factor, allocator);
  try_load_TextureInfo(*it, "baseColorTexture", &ti->base_color_texture,
                       allocator);
  try_load_float(*it, "metallicFactor", ti->metallic_factor);
  try_load_TextureInfo(*it, "metallicRoughnessTexture",
                       &ti->metallic_roughness_texture, allocator);
  try_load_float(*it, "roughnessFactor", ti->roughness_factor);

  *texture_info = ti;
}

static void load_material(json& json_data, glTF::Material& material,
                          Allocator* allocator) {
  try_load_float_array(json_data, "emissiveFactor",
                       material.emissive_factor_count,
  ...
  try_load_TextureInfo(json_data, "emissiveTexture", &material.emissive_texture,
                       allocator);
  try_load_MaterialNormalTextureInfo(json_data, "normalTexture",
                                     &material.normal_texture, allocator);
  try_load_MaterialOcclusionTextureInfo(json_data, "occlusionTexture",
                                        &material.occlusion_texture, allocator);
...
static void load_materials(json& json_data, glTF::glTF& gltf_data,
                           Allocator* allocator) {
  json array = json_data["materials"];

  sizet array_count = array.size();
  gltf_data.materials = (glTF::Material*)allocate_and_zero(
      allocator, sizeof(glTF::Material) * array_count);
...
static void load_texture(json& json_data, glTF::Texture& texture,
                         Allocator* allocator) {
...
static void load_textures(json& json_data, glTF::glTF& gltf_data,
                          Allocator* allocator) {
  json array = json_data["textures"];

  sizet array_count = array.size();
  gltf_data.textures = (glTF::Texture*)allocate_and_zero(
      allocator, sizeof(glTF::Texture) * array_count);
...
static void load_image(json& json_data, glTF::Image& image,
                       Allocator* allocator) {
...
static void load_images(json& json_data, glTF::glTF& gltf_data,
                        Allocator* allocator) {
  json array = json_data["images"];

  sizet array_count = array.size();
  gltf_data.images = (glTF::Image*)allocate_and_zero(
      allocator, sizeof(glTF::Image) * array_count);
...
static void load_sampler(json& json_data, glTF::Sampler& sampler,
                         Allocator* allocator) {
...
static void load_samplers(json& json_data, glTF::glTF& gltf_data,
                          Allocator* allocator) {
  json array = json_data["samplers"];

  sizet array_count = array.size();
  gltf_data.samplers = (glTF::Sampler*)allocate_and_zero(
      allocator, sizeof(glTF::Sampler) * array_count);
...
static void load_skin(json& json_data, glTF::Skin& skin, Allocator* allocator) {
  ...
  try_load_int(json_data, "inverseBindMatrices",
               skin.inverse_bind_matrices_buffer_index);
  try_load_int_array(json_data, "joints", skin.joints_count, &skin.joints,
                     allocator);
}

static void load_skins(json& json_data, glTF::glTF& gltf_data,
                       Allocator* allocator) {
  json array = json_data["skins"];

  sizet array_count = array.size();
  gltf_data.skins = (glTF::Skin*)allocate_and_zero(
      allocator, sizeof(glTF::Skin) * array_count);
...
static void load_animation(json& json_data, glTF::Animation& animation,
                           Allocator* allocator) {
...
static void load_animations(json& json_data, glTF::glTF& gltf_data,
                            Allocator* allocator) {
  json array = json_data["animations"];

  sizet array_count = array.size();
  gltf_data.animations = (glTF::Animation*)allocate_and_zero(
      allocator, sizeof(glTF::Animation) * array_count);
...
i32 raptor::glTF::get_data_offset(i32 accessor_offset, i32 buffer_view_offset) {
  ...
  i32 byte_offset =
      buffer_view_offset == INVALID_INT_VALUE ? 0 : buffer_view_offset;
