...
static bool control_is_empty(i8 control) {
  return control == k_control_bitmask_empty;
}

static bool control_is_full(i8 control) {
  return control >= 0;
}

static bool control_is_deleted(i8 control) {
  return control == k_control_bitmask_deleted;
}

static bool control_is_empty_or_deleted(i8 control) {
  return control < k_control_bitmask_sentinel;
}
...
static u64 hash_seed(const i8* control) {
  return reinterpret_cast<uintptr_t>(control) >> 12;
}

static u64 hash_1(u64 hash, const i8* ctrl) {
  return (hash >> 7) ^ hash_seed(ctrl);
}

static i8 hash_2(u64 hash) {
  return hash & 0x7F;
}
...
static void ConvertDeletedToEmptyAndFullToDeleted(i8* ctrl, size_t capacity) {
  ...
  for (i8* pos = ctrl; pos != ctrl + capacity + 1;
       pos += GroupSse2Impl::kWidth) {
...
void FlatHashMap<K, V>::reset_ctrl() {
  memset(control_bytes, k_control_bitmask_empty,
         capacity + GroupSse2Impl::kWidth);
...
template <typename K, typename V>
inline void FlatHashMap<K, V>::init(Allocator* allocator_,
...
void FlatHashMap<K, V>::erase_meta(const FlatHashMapIterator& iterator) {
  ...
  const auto empty_before =
      GroupSse2Impl(control_bytes + index_before).MatchEmpty();
  ...
  set_ctrl(index, was_never_full ? k_control_bitmask_empty
                                 : k_control_bitmask_deleted);
...
u64 FlatHashMap<K, V>::prepare_insert(u64 hash) {
  ...
  if (growth_left == 0 &&
      !control_is_deleted(control_bytes[find_info.offset])) {
...
void FlatHashMap<K, V>::drop_deletes_without_resize() {
  ...
  for (size_t i = 0; i != capacity; ++i) {
    ...
    const auto probe_index = [&](size_t pos) {
      return ((pos - probe(hash).get_offset()) & capacity) /
             GroupSse2Impl::kWidth;
    };
...
void FlatHashMap<K, V>::initialize_slots() {
  ...
  slots_ = reinterpret_cast<KeyValue*>(new_memory + capacity +
                                       GroupSse2Impl::kWidth);
...
void FlatHashMap<K, V>::set_ctrl(u64 i, i8 h) {
  ...
  control_bytes[((i - kClonedBytes) & capacity) + (kClonedBytes & capacity)] =
      h;
}
...
template <typename K, typename V>
inline void FlatHashMap<K, V>::iterator_skip_empty_or_deleted(
...
// Capacity ///////////////////////////////////////////////////////////
bool capacity_is_valid(size_t n) {
  return ((n + 1) & n) == 0 && n > 0;
}
...
// Rounds up the capacity to the next power of 2 minus 1, with a minimum of 1.
u64 capacity_normalize(u64 n) {
  return n ? ~u64{} >> lzcnt_soft(n) : 1;
}

//
u64 capacity_to_growth(u64 capacity) {
