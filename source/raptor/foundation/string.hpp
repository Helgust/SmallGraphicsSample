...
struct StringBuffer {
  ...
  char* append_use(
      const StringView&
          text);  // Append and returns a pointer to the start. Used for strings mostly.
  char* append_use_substring(
      const char* string, u32 start_index,
...
struct StringArray {
  ...
  FlatHashMap<u64, u32>*
      string_to_index;  // Note: trying to avoid bringing the hash map header.
