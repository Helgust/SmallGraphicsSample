...
sizet file_write(uint8_t* memory, u32 element_size, u32 count,
                 FileHandle file) {
...
void file_open_directory(cstring path, Directory* out_directory) {
  ...
  HANDLE found_handle;
  if ((found_handle = FindFirstFileA(out_directory->path, &find_data)) !=
...
void file_find_files_in_path(cstring file_pattern, StringArray& files) {
  ...
  WIN32_FIND_DATAA find_data;
  HANDLE hFind;
  if ((hFind = FindFirstFileA(file_pattern, &find_data)) !=
      INVALID_HANDLE_VALUE) {
    do {
...
void file_find_files_in_path(cstring extension, cstring search_pattern,
  ...
  WIN32_FIND_DATAA find_data;
  HANDLE hFind;
  if ((hFind = FindFirstFileA(search_pattern, &find_data)) !=
      INVALID_HANDLE_VALUE) {
    do {
