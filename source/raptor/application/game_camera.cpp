...
vec3s lerp3(const vec3s& from, const vec3s& to, f32 t, f32 dt) {
  return vec3s{lerp(from.x, to.x, t, dt), lerp(from.y, to.y, t, dt),
               lerp(from.z, to.z, t, dt)};
}

void GameCamera::update(InputService* input, u32 window_width,
                        u32 window_height, f32 delta_time) {
  ...
  if (input->is_mouse_dragging(MOUSE_BUTTONS_RIGHT) &&
      !ImGui::IsAnyItemHovered()) {

    if (ignore_dragging_frames == 0) {
      target_yaw -= (input->mouse_position.x - roundu32(window_width / 2.f)) *
                    mouse_sensitivity * delta_time;
      target_pitch -=
          (input->mouse_position.y - roundu32(window_height / 2.f)) *
          mouse_sensitivity * delta_time;
    } else {
  ...
  if (input->is_key_down(KEY_UP) || input->is_key_down(KEY_W)) {
    camera_movement =
        glms_vec3_add(camera_movement,
                      glms_vec3_scale(camera.direction, camera_movement_delta));
...

  {
    ...
    camera.position =
        lerp3(camera.position, target_movement, 0.9f, tween_position_speed);
  }
}
