...
void Window::init(void* configuration_) {
  ...
  window = SDL_CreateWindow(configuration.name, SDL_WINDOWPOS_CENTERED,
...
void Window::register_os_messages_callback(OsMessagesCallback callback,
                                           void* user_data) {
...
void Window::unregister_os_messages_callback(OsMessagesCallback callback) {
  RASSERTM(os_messages_callbacks.size < 8,
           "This array is too big for a linear search. Consider using "
           "something different!");
...
void Window::center_mouse(bool dragging) {
  if (dragging) {
    SDL_WarpMouseInWindow(window, raptor::roundu32(width / 2.f),
                          raptor::roundu32(height / 2.f));
