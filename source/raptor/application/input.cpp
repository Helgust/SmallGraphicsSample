...
struct InputBackend {
  ...
  void on_event(void* event_, u8* keys, u32 num_keys, Gamepad* gamepads,
...
void InputBackend::get_mouse_state(InputVector2& position, u8* buttons,
                                   u32 num_buttons) {
  i32 mouse_x, mouse_y;
...
void InputBackend::on_event(void* event_, u8* keys, u32 num_keys,
                            Gamepad* gamepads, u32 num_gamepads,
                            bool& has_focus) {
    ...
    case SDL_CONTROLLERBUTTONUP: {
      ...
      for (size_t i = 0; i < k_max_gamepads; i++) {
        if (gamepads[i].id == event.cbutton.which) {
          gamepads[i].buttons[event.cbutton.button] =
...
InputHandle InputService::create_action_map(
    const InputActionMapCreation& creation) {
...
void InputService::add_button(InputHandle action, DevicePart device_part,
                              uint16_t button, bool repeat) {
  const InputAction& binding_action = actions[action];

  InputBinding binding;
  binding
      .set(BINDING_TYPE_BUTTON, device_from_part(device_part), device_part,
           button, 0, 0, repeat)
...
void InputService::add_axis_1d(InputHandle action, DevicePart device_part,
                               uint16_t axis, float min_deadzone,
                               float max_deadzone) {
  const InputAction& binding_action = actions[action];

  InputBinding binding;
  binding
      .set(BINDING_TYPE_AXIS_1D, device_from_part(device_part), device_part,
           axis, 0, 0, 0)
...
void InputService::add_axis_2d(InputHandle action, DevicePart device_part,
  ...
  binding
      .set(BINDING_TYPE_AXIS_2D, device_from_part(device_part), device_part,
           u16_max, 1, 0, 0)
      .set_deadzones(min_deadzone, max_deadzone)
      .set_handles(binding_action.action_map, action);
  binding_x
...
void InputService::add_vector_1d(InputHandle action, DevicePart device_part_pos,
                                 uint16_t button_pos,
                                 DevicePart device_part_neg,
                                 uint16_t button_neg, bool repeat) {
  const InputAction& binding_action = actions[action];

  InputBinding binding, binding_positive, binding_negative;
  binding
      .set(BINDING_TYPE_VECTOR_1D, device_from_part(device_part_pos),
           device_part_pos, u16_max, 1, 0, repeat)
      .set_handles(binding_action.action_map, action);
  binding_positive
      .set(BINDING_TYPE_VECTOR_1D, device_from_part(device_part_pos),
           device_part_pos, button_pos, 0, 1, repeat)
      .set_handles(binding_action.action_map, action);
  binding_negative
      .set(BINDING_TYPE_VECTOR_1D, device_from_part(device_part_neg),
           device_part_neg, button_neg, 0, 1, repeat)
      .set_handles(binding_action.action_map, action);
...
void InputService::add_vector_2d(
    InputHandle action, DevicePart device_part_up, uint16_t button_up,
  ...
  binding
      .set(BINDING_TYPE_VECTOR_2D, device_from_part(device_part_up),
           device_part_up, u16_max, 1, 0, repeat)
      .set_handles(binding_action.action_map, action);
  binding_up
      .set(BINDING_TYPE_VECTOR_2D, device_from_part(device_part_up),
           device_part_up, button_up, 0, 1, repeat)
      .set_handles(binding_action.action_map, action);
  binding_down
      .set(BINDING_TYPE_VECTOR_2D, device_from_part(device_part_down),
           device_part_down, button_down, 0, 1, repeat)
      .set_handles(binding_action.action_map, action);
  binding_left
      .set(BINDING_TYPE_VECTOR_2D, device_from_part(device_part_left),
           device_part_left, button_left, 0, 1, repeat)
      .set_handles(binding_action.action_map, action);
  binding_right
      .set(BINDING_TYPE_VECTOR_2D, device_from_part(device_part_right),
           device_part_right, button_right, 0, 1, repeat)
      .set_handles(binding_action.action_map, action);
...
void InputService::update(f32 delta) {
  ...
  s_input_backend.get_mouse_state(mouse_position, mouse_button,
                                  MOUSE_BUTTONS_COUNT);
      ...
      case DEVICE_GAMEPAD: {
        ...
        switch (input_binding.device_part) {
          ...
          case DEVICE_PART_GAMEPAD_BUTTONS: {
            ...
            input_binding.value =
                input_binding.repeat
                    ? gamepad.is_button_down(
                          (GamepadButtons)input_binding.button)
                    : gamepad.is_button_just_pressed(
                          (GamepadButtons)input_binding.button);
            break;
          }
            /*case InputBinding::GAMEPAD_HAT:
    ...
    switch (input_binding.type) {
      case BINDING_TYPE_BUTTON: {
        input_action.value.x =
            fmax(input_action.value.x, input_binding.value ? 1.0f : 0.0f);
        break;
      }

      case BINDING_TYPE_AXIS_1D: {
        input_action.value.x = input_binding.value != 0.f
                                   ? input_binding.value
                                   : input_action.value.x;
        break;
      }
      ...
      case BINDING_TYPE_AXIS_2D: {
        ...
        input_action.value.x = input_binding_x.value != 0.0f
                                   ? input_binding_x.value
                                   : input_action.value.x;
        input_action.value.y = input_binding_y.value != 0.0f
                                   ? input_binding_y.value
      ...
      case BINDING_TYPE_VECTOR_2D: {
        ...
        input_action.value.x = input_binding_right.value ? 1.0f
                               : input_binding_left.value
                                   ? -1.0f
                                   : input_action.value.x;
        input_action.value.y = input_binding_up.value ? 1.0f
                               : input_binding_down.value
                                   ? -1.0f
                                   : input_action.value.y;
        break;
      }
  ...
  if (ImGui::Begin("Input")) {
    ...
    if (ImGui::TreeNode("Devices")) {
      ...
      if (ImGui::TreeNode("Mouse")) {
        ...
        ImGui::Text("Previous pos %f,%f", previous_mouse_position.x,
        ...
        for (u32 i = 0; i < MOUSE_BUTTONS_COUNT; i++) {
          ...
          ImGui::SameLine();
          ImGui::Text("Clicked Position     %4.1f,%4.1f",
                      mouse_clicked_position[i].x, mouse_clicked_position[i].y);
          ImGui::SameLine();
          ImGui::Text("Button %u, Previous %u", mouse_button[i],
                      previous_mouse_button[i]);
          ImGui::SameLine();
    ...
    if (ImGui::TreeNode("Actions")) {
      ...
      for (u32 j = 0; j < actions.size; j++) {
        ...
        ImGui::Text("Action %s, x %2.3f y %2.3f", input_action.name,
                    input_action.value.x, input_action.value.y);
      }
    ...
    if (ImGui::TreeNode("Bindings")) {
      for (u32 k = 0; k < bindings.size; k++) {
        ...
        switch (binding.type) {
          case BINDING_TYPE_VECTOR_1D: {
            ImGui::Text(
                "Binding action %s, type %s, value %f, composite %u, part of "
                "composite %u, button %s",
                parent_action.name, "vector 1d", binding.value,
                binding.is_composite, binding.is_part_of_composite,
                button_name);
            break;
          }
          case BINDING_TYPE_VECTOR_2D: {
            ImGui::Text(
                "Binding action %s, type %s, value %f, composite %u, part of "
                "composite %u",
                parent_action.name, "vector 2d", binding.value,
                binding.is_composite, binding.is_part_of_composite);
            break;
          }
          case BINDING_TYPE_AXIS_1D: {
            ImGui::Text(
                "Binding action %s, type %s, value %f, composite %u, part of "
                "composite %u",
                parent_action.name, "axis 1d", binding.value,
                binding.is_composite, binding.is_part_of_composite);
            break;
          }
          case BINDING_TYPE_AXIS_2D: {
            ImGui::Text(
                "Binding action %s, type %s, value %f, composite %u, part of "
                "composite %u",
                parent_action.name, "axis 2d", binding.value,
                binding.is_composite, binding.is_part_of_composite);
            break;
          }
          case BINDING_TYPE_BUTTON: {
            ImGui::Text(
                "Binding action %s, type %s, value %f, composite %u, part of "
                "composite %u, button %s",
                parent_action.name, "button", binding.value,
                binding.is_composite, binding.is_part_of_composite,
                button_name);
            break;
          }
...
InputBinding& InputBinding::set(BindingType type_, Device device_,
                                DevicePart device_part_, u16 button_,
                                u8 is_composite_, u8 is_part_of_composite_,
                                u8 repeat_) {
  type = type_;
  device = device_;
...
InputBinding& InputBinding::set_handles(InputHandle action_map,
                                        InputHandle action) {
