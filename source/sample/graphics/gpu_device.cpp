...
#define check(result) \
  RASSERTM(result == VK_SUCCESS, "Vulkan assert code %u", result)
...
struct CommandBufferRing {
  ...
  static u16 pool_from_index(u32 index) {
    return (u16)index / k_buffer_per_pool;
  }
...
void CommandBufferRing::init(GpuDevice* gpu_) {
  ...
  for (u32 i = 0; i < k_max_pools; i++) {
    ...
    check(vkCreateCommandPool(gpu->vulkan_device, &cmd_pool_info,
  ...
  for (u32 i = 0; i < k_max_buffers; i++) {
    ...
    check(vkAllocateCommandBuffers(gpu->vulkan_device, &cmd,
...
void CommandBufferRing::reset_pools(u32 frame_index) {
  ...
  for (u32 i = 0; i < k_max_threads; i++) {
    vkResetCommandPool(gpu->vulkan_device,
                       vulkan_command_pools[frame_index * k_max_threads + i],
                       0);
  }
}
...
bool GpuDevice::get_family_queue(VkPhysicalDevice physical_device) {
  ...
  for (; family_index < queue_family_count; ++family_index) {
    ...
    if (queue_family.queueCount > 0 &&
      ...
      vkGetPhysicalDeviceSurfaceSupportKHR(physical_device, family_index,
...
void GpuDevice::init(const DeviceCreation& creation) {
  ...
  result = vkCreateInstance(&create_info, vulkan_allocation_callbacks,
                            &vulkan_instance);
  check(result);
...
#ifdef VULKAN_DEBUG_REPORT
  {
    ...
    VkExtensionProperties* extensions = (VkExtensionProperties*)ralloca(
        sizeof(VkExtensionProperties) * num_instance_extensions, allocator);
    vkEnumerateInstanceExtensionProperties(nullptr, &num_instance_extensions,
                                           extensions);
    ...
    if (!debug_utils_extension_present) {
      rprint("Extension %s for debugging non present.",
             VK_EXT_DEBUG_UTILS_EXTENSION_NAME);
    } else {
      ...
      vkCreateDebugUtilsMessengerEXT(
          vulkan_instance, &debug_messenger_create_info,
  ...
  result =
      vkEnumeratePhysicalDevices(vulkan_instance, &num_physical_device, NULL);
  check(result);

  VkPhysicalDevice* gpus = (VkPhysicalDevice*)ralloca(
      sizeof(VkPhysicalDevice) * num_physical_device, allocator);
  result =
      vkEnumeratePhysicalDevices(vulkan_instance, &num_physical_device, gpus);
  check(result);

  //////// Create drawable surface
  // Create surface
  SDL_Window* window = (SDL_Window*)creation.window;
  if (SDL_Vulkan_CreateSurface(window, vulkan_instance,
                               &vulkan_window_surface) == SDL_FALSE) {
  ...
  for (u32 i = 0; i < num_physical_device; ++i) {
    ...
    if (vulkan_physical_properties.deviceType ==
  ...
  vkGetPhysicalDeviceProperties(vulkan_physical_device,
                                &vulkan_physical_properties);
  gpu_timestamp_frequency =
      vulkan_physical_properties.limits.timestampPeriod / (1000 * 1000);

  rprint("GPU Used: %s\n", vulkan_physical_properties.deviceName);

  s_ubo_alignment =
      vulkan_physical_properties.limits.minUniformBufferOffsetAlignment;
  s_ssbo_alignemnt =
      vulkan_physical_properties.limits.minStorageBufferOffsetAlignment;
  ...
  vkGetPhysicalDeviceSurfaceFormatsKHR(
      vulkan_physical_device, vulkan_window_surface, &supported_count, NULL);
  VkSurfaceFormatKHR* supported_formats = (VkSurfaceFormatKHR*)ralloca(
      sizeof(VkSurfaceFormatKHR) * supported_count, allocator);
  vkGetPhysicalDeviceSurfaceFormatsKHR(vulkan_physical_device,
                                       vulkan_window_surface, &supported_count,
                                       supported_formats);
  ...
  result = vkCreateDescriptorPool(vulkan_device, &pool_info,
  ...
  VkQueryPoolCreateInfo vqpci{
      ...
      creation.gpu_time_queries_per_frame * 2u * k_max_frames,
      0};
  vkCreateQueryPool(vulkan_device, &vqpci, vulkan_allocation_callbacks,
                    &vulkan_timestamp_query_pool);
  ...
  u8* memory = rallocam(
      sizeof(GPUTimestampManager) + sizeof(CommandBuffer*) * 128, allocator);
  ...
  for (size_t i = 0; i < k_max_swapchain_images; i++) {
    ...
    vkCreateSemaphore(vulkan_device, &semaphore_info,
                      vulkan_allocation_callbacks,
                      &vulkan_image_acquired_semaphore[i]);
  ...
  gpu_timestamp_manager->init(allocator, creation.gpu_time_queries_per_frame,
                              k_max_frames);
  ...
  CommandBuffer** correctly_allocated_buffer =
      (CommandBuffer**)(memory + sizeof(GPUTimestampManager));
  RASSERTM(queued_command_buffers == correctly_allocated_buffer,
           "Wrong calculations for queued command buffers arrays. Should be "
           "%p, but it is %p.",
  ...
  BufferCreation fullscreen_vb_creation = {VK_BUFFER_USAGE_VERTEX_BUFFER_BIT,
                                           ResourceUsageType::Immutable, 0,
                                           nullptr, "Fullscreen_vb"};
  fullscreen_vertex_buffer = create_buffer(fullscreen_vb_creation);

  // Create depth image
  TextureCreation depth_texture_creation = {nullptr,
                                            swapchain_width,
                                            swapchain_height,
                                            1,
                                            1,
  ...
  swapchain_pass_creation.set_type(RenderPassType::Swapchain)
      .set_name("Swapchain");
  swapchain_pass_creation.set_operations(RenderPassOperation::Clear,
  ...
  TextureCreation dummy_texture_creation = {
      nullptr, 1, 1, 1, 1, 0, VK_FORMAT_R8_UINT, TextureType::Texture2D};
  dummy_texture = create_texture(dummy_texture_creation);

  BufferCreation dummy_constant_buffer_creation = {
      VK_BUFFER_USAGE_UNIFORM_BUFFER_BIT, ResourceUsageType::Immutable, 16,
      nullptr, "Dummy_cb"};
...
void GpuDevice::shutdown() {
  ...
  for (size_t i = 0; i < k_max_swapchain_images; i++) {
    vkDestroySemaphore(vulkan_device, vulkan_render_complete_semaphore[i],
                       vulkan_allocation_callbacks);
    vkDestroySemaphore(vulkan_device, vulkan_image_acquired_semaphore[i],
                       vulkan_allocation_callbacks);
  ...
  while (it.is_valid()) {
    ...
    vkDestroyRenderPass(vulkan_device, vk_render_pass,
                        vulkan_allocation_callbacks);
  ...
  vkDestroyRenderPass(vulkan_device, vk_swapchain_pass->vk_render_pass,
                      vulkan_allocation_callbacks);

  // Destroy swapchain
  destroy_swapchain();
  vkDestroySurfaceKHR(vulkan_instance, vulkan_window_surface,
                      vulkan_allocation_callbacks);
  ...
  vkDestroyDescriptorPool(vulkan_device, vulkan_descriptor_pool,
                          vulkan_allocation_callbacks);
  vkDestroyQueryPool(vulkan_device, vulkan_timestamp_query_pool,
                     vulkan_allocation_callbacks);
...
static void transition_image_layout(VkCommandBuffer command_buffer,
                                    ...
                                    VkImageLayout newLayout, bool is_depth) {
...
// Resource Creation ////////////////////////////////////////////////////////////
static void vulkan_create_texture(GpuDevice& gpu,
  ...
  const bool is_render_target =
      (creation.flags & TextureFlags::RenderTarget_mask) ==
      TextureFlags::RenderTarget_mask;
  const bool is_compute_used = (creation.flags & TextureFlags::Compute_mask) ==
  ...
  if (TextureFormat::has_depth_or_stencil(creation.format)) {
    ...
    image_info.usage |=
        is_render_target ? VK_IMAGE_USAGE_COLOR_ATTACHMENT_BIT : 0;
  }
  ...
  gpu.set_resource_name(VK_OBJECT_TYPE_IMAGE, (u64)texture->vk_image,
                        creation.name);
  ...
  if (TextureFormat::has_depth_or_stencil(creation.format)) {
    ...
    info.subresourceRange.aspectMask = TextureFormat::has_depth(creation.format)
  ...
  check(vkCreateImageView(gpu.vulkan_device, &info,
                          gpu.vulkan_allocation_callbacks,
                          &texture->vk_image_view));

  gpu.set_resource_name(VK_OBJECT_TYPE_IMAGE_VIEW, (u64)texture->vk_image_view,
                        creation.name);
...
TextureHandle GpuDevice::create_texture(const TextureCreation& creation) {
  ...
  if (creation.initial_data) {
    ...
    memcpy(destination_data, creation.initial_data,
    ...
    // Transition
    transition_image_layout(command_buffer->vk_command_buffer,
                            texture->vk_image, texture->vk_format,
...
void dump_shader_code(StringBuffer& temp_string_buffer, cstring code,
  ...
  while (current_code) {
    ...
    char* line = temp_string_buffer.append_use_substring(
        current_code, 0, (end_of_line - current_code));
...
VkShaderModuleCreateInfo GpuDevice::compile_shader(cstring code, u32 code_size,
  ...
  char* stage_define =
      temp_string_buffer.append_use_f("%s_%s", to_stage_defines(stage), name);
  ...
  // Compile to SPV
#if defined(_MSC_VER)
  char* glsl_compiler_path = temp_string_buffer.append_use_f(
      "%sglslangValidator.exe", vulkan_binaries_path);
  char* final_spirv_filename =
      temp_string_buffer.append_use("shader_final.spv");
  // TODO: add optional debug information in shaders (option -g).
  char* arguments = temp_string_buffer.append_use_f(
      "glslangValidator.exe %s -V --target-env vulkan1.2 -o %s -S %s --D %s "
      "--D %s",
      temp_filename, final_spirv_filename, to_compiler_extension(stage),
      stage_define, to_stage_defines(stage));
#else
  char* glsl_compiler_path = temp_string_buffer.append_use_f(
      "%sglslangValidator", vulkan_binaries_path);
  char* final_spirv_filename =
      temp_string_buffer.append_use("shader_final.spv");
  char* arguments = temp_string_buffer.append_use_f(
      "%s -V --target-env vulkan1.2 -o %s -S %s --D %s --D %s", temp_filename,
      final_spirv_filename, to_compiler_extension(stage), stage_define,
      to_stage_defines(stage));
#endif
  ...
  if (optimize_shaders) {
    ...
    char* spirv_optimizer_path = temp_string_buffer.append_use_f(
        "%sspirv-opt.exe", vulkan_binaries_path);
    char* optimized_spirv_filename =
        temp_string_buffer.append_use_f("shader_opt.spv");
    char* spirv_opt_arguments = temp_string_buffer.append_use_f(
        "spirv-opt.exe -O --preserve-bindings %s -o %s", final_spirv_filename,
        optimized_spirv_filename);

    process_execute(".", spirv_optimizer_path, spirv_opt_arguments, "");

    // Read back SPV file.
    shader_create_info.pCode = reinterpret_cast<const u32*>(
        file_read_binary(optimized_spirv_filename, temporary_allocator,
    ...
    // Read back SPV file.
    shader_create_info.pCode = reinterpret_cast<const u32*>(
        file_read_binary(final_spirv_filename, temporary_allocator,
                         &shader_create_info.codeSize));
  }
...
ShaderStateHandle GpuDevice::create_shader_state(
    const ShaderStateCreation& creation) {
  ...
  for (compiled_shaders = 0; compiled_shaders < creation.stages_count;
       ++compiled_shaders) {
    ...
    if (creation.spv_input) {
      ...
      shader_create_info = compile_shader(stage.code, stage.code_size,
                                          stage.type, creation.name);
    }
    ...
    if (vkCreateShaderModule(
            vulkan_device, &shader_create_info, nullptr,
    ...
    set_resource_name(
        ...
        (u64)shader_state->shader_stage_info[compiled_shaders].module,
        creation.name);
  }
  // Not needed anymore - temp allocator freed at the end.
...
PipelineHandle GpuDevice::create_pipeline(const PipelineCreation& creation) {
  ...
  for (u32 l = 0; l < creation.num_active_layouts; ++l) {
    pipeline->descriptor_set_layout[l] =
        access_descriptor_set_layout(creation.descriptor_set_layout[l]);
    pipeline->descriptor_set_layout_handle[l] =
        creation.descriptor_set_layout[l];

    vk_layouts[l] =
        pipeline->descriptor_set_layout[l]->vk_descriptor_set_layout;
  }
  ...
  if (shader_state_data->graphics_pipeline) {
    ...
    if (creation.vertex_input.num_vertex_attributes) {
      ...
      for (u32 i = 0; i < creation.vertex_input.num_vertex_attributes; ++i) {
        const VertexAttribute& vertex_attribute =
            creation.vertex_input.vertex_attributes[i];
        vertex_attributes[i] = {vertex_attribute.location,
    ...
    if (creation.vertex_input.num_vertex_streams) {
      ...
      for (u32 i = 0; i < creation.vertex_input.num_vertex_streams; ++i) {
        const VertexStream& vertex_stream =
            creation.vertex_input.vertex_streams[i];
        VkVertexInputRate vertex_rate =
            vertex_stream.input_rate == VertexInputRate::PerVertex
                ? VkVertexInputRate::VK_VERTEX_INPUT_RATE_VERTEX
                : VkVertexInputRate::VK_VERTEX_INPUT_RATE_INSTANCE;
        vertex_bindings[i] = {vertex_stream.binding, vertex_stream.stride,
                              vertex_rate};
      }
    ...
    if (creation.blend_state.active_states) {
      for (size_t i = 0; i < creation.blend_state.active_states; i++) {
        ...
        color_blend_attachment[i].blendEnable =
            blend_state.blend_enabled ? VK_TRUE : VK_FALSE;
        color_blend_attachment[i].srcColorBlendFactor =
            blend_state.source_color;
        color_blend_attachment[i].dstColorBlendFactor =
            blend_state.destination_color;
        ...
        if (blend_state.separate_blend) {
          color_blend_attachment[i].srcAlphaBlendFactor =
              blend_state.source_alpha;
          color_blend_attachment[i].dstAlphaBlendFactor =
              blend_state.destination_alpha;
    ...
    depth_stencil.depthWriteEnable =
        creation.depth_stencil.depth_write_enable ? VK_TRUE : VK_FALSE;
    depth_stencil.stencilTestEnable =
        creation.depth_stencil.stencil_enable ? VK_TRUE : VK_FALSE;
    depth_stencil.depthTestEnable =
        creation.depth_stencil.depth_enable ? VK_TRUE : VK_FALSE;
    ...
    pipeline_info.renderPass =
        get_vulkan_render_pass(creation.render_pass, creation.name);
    ...
    vkCreateGraphicsPipelines(vulkan_device, VK_NULL_HANDLE, 1, &pipeline_info,
                              vulkan_allocation_callbacks,
    ...
    vkCreateComputePipelines(vulkan_device, VK_NULL_HANDLE, 1, &pipeline_info,
                             vulkan_allocation_callbacks,
                             &pipeline->vk_pipeline);
...
BufferHandle GpuDevice::create_buffer(const BufferCreation& creation) {
  ...
  static const VkBufferUsageFlags k_dynamic_buffer_mask =
      VK_BUFFER_USAGE_VERTEX_BUFFER_BIT | VK_BUFFER_USAGE_INDEX_BUFFER_BIT |
      VK_BUFFER_USAGE_UNIFORM_BUFFER_BIT;
  const bool use_global_buffer =
      (creation.type_flags & k_dynamic_buffer_mask) != 0;
  ...
  buffer_info.size = creation.size > 0
                         ? creation.size
                         : 1;  // 0 sized creations are not permitted.
  ...
  set_resource_name(VK_OBJECT_TYPE_BUFFER, (u64)buffer->vk_buffer,
                    creation.name);
...
SamplerHandle GpuDevice::create_sampler(const SamplerCreation& creation) {
  ...
  vkCreateSampler(vulkan_device, &create_info, vulkan_allocation_callbacks,
                  &sampler->vk_sampler);

  set_resource_name(VK_OBJECT_TYPE_SAMPLER, (u64)sampler->vk_sampler,
                    creation.name);
...
DescriptorSetLayoutHandle GpuDevice::create_descriptor_set_layout(
  ...
  DesciptorSetLayout* descriptor_set_layout =
      access_descriptor_set_layout(handle);
  ...
  for (u32 r = 0; r < creation.num_bindings; ++r) {
    ...
    const DescriptorSetLayoutCreation::Binding& input_binding =
        creation.bindings[r];
    binding.start =
        input_binding.start == u16_max ? (u16)r : input_binding.start;
    binding.count = 1;
    binding.type = input_binding.type;
    binding.name = input_binding.name;

    VkDescriptorSetLayoutBinding& vk_binding =
        descriptor_set_layout->vk_binding[used_bindings];
    ++used_bindings;
...
//
//
static void vulkan_fill_write_descriptor_sets(
  ...
  for (u32 r = 0; r < num_resources; r++) {
    ...
    switch (binding.type) {
      case VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER: {
        ...
        image_info[i].imageLayout =
            TextureFormat::has_depth_or_stencil(texture_data->vk_format)
      ...
      case VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER: {
        ...
        descriptor_write[i].descriptorType =
            buffer->usage == ResourceUsageType::Dynamic
                ? VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER_DYNAMIC
      ...
      default: {
        RASSERTM(false,
                 "Resource type %d not supported in descriptor set creation!\n",
                 binding.type);
        break;
      }
    }
...
DescriptorSetHandle GpuDevice::create_descriptor_set(
  ...
  check(vkAllocateDescriptorSets(vulkan_device, &alloc_info,
                                 &descriptor_set->vk_descriptor_set));
  // Cache data
  u8* memory =
      rallocam((sizeof(ResourceHandle) + sizeof(SamplerHandle) + sizeof(u16)) *
  ...
  vulkan_fill_write_descriptor_sets(
      *this, descriptor_set_layout, descriptor_set->vk_descriptor_set,
  ...
  vkUpdateDescriptorSets(vulkan_device, num_resources, descriptor_write, 0,
                         nullptr);

  return handle;
}

static void vulkan_create_swapchain_pass(GpuDevice& gpu,
  ...
  check(vkCreateRenderPass(gpu.vulkan_device, &render_pass_info, nullptr,
                           &render_pass->vk_render_pass));

  gpu.set_resource_name(VK_OBJECT_TYPE_RENDER_PASS,
                        (u64)render_pass->vk_render_pass, creation.name);
  ...
  for (size_t i = 0; i < gpu.vulkan_swapchain_image_count; i++) {
    ...
    vkCreateFramebuffer(gpu.vulkan_device, &framebuffer_info, nullptr,
                        &gpu.vulkan_swapchain_framebuffers[i]);
    gpu.set_resource_name(VK_OBJECT_TYPE_FRAMEBUFFER,
                          (u64)gpu.vulkan_swapchain_framebuffers[i],
                          creation.name);
  }
  ...
  for (size_t i = 0; i < gpu.vulkan_swapchain_image_count; i++) {
    transition_image_layout(
        command_buffer->vk_command_buffer, gpu.vulkan_swapchain_images[i],
        gpu.vulkan_surface_format.format, VK_IMAGE_LAYOUT_UNDEFINED,
...
static void vulkan_create_framebuffer(GpuDevice& gpu, RenderPass* render_pass,
                                      const TextureHandle* output_textures,
  ...
  for (; active_attachments < num_render_targets; ++active_attachments) {
    Texture* texture_vk =
        gpu.access_texture(output_textures[active_attachments]);
  ...
  if (depth_stencil_texture.index != k_invalid_index) {
    ...
    framebuffer_attachments[active_attachments++] =
        depth_texture_vk->vk_image_view;
  }
  framebuffer_info.pAttachments = framebuffer_attachments;
  ...
  gpu.set_resource_name(VK_OBJECT_TYPE_FRAMEBUFFER,
                        (u64)render_pass->vk_frame_buffer, render_pass->name);
}

//
//
static VkRenderPass vulkan_create_render_pass(GpuDevice& gpu,
  ...
  subpass.colorAttachmentCount =
      active_attachments ? active_attachments - 1 : 0;
  ...
  render_pass_info.attachmentCount =
      (active_attachments ? active_attachments - 1 : 0) + depth_stencil_count;
  render_pass_info.pAttachments = attachments;
  render_pass_info.subpassCount = 1;
  render_pass_info.pSubpasses = &subpass;
...
//
//
static RenderPassOutput fill_render_pass_output(
    GpuDevice& gpu, const RenderPassCreation& creation) {
...
RenderPassHandle GpuDevice::create_render_pass(
    const RenderPassCreation& creation) {
  ...
  switch (creation.type) {
    ...
    case RenderPassType::Geometry: {
      ...
      render_pass->vk_render_pass =
          get_vulkan_render_pass(render_pass->output, creation.name);

      vulkan_create_framebuffer(*this, render_pass, creation.output_textures,
...
void GpuDevice::destroy_buffer(BufferHandle buffer) {
  if (buffer.index < buffers.pool_size) {
    resource_deletion_queue.push(
        {ResourceDeletionType::Buffer, buffer.index, current_frame});
  } else {
...
void GpuDevice::destroy_pipeline(PipelineHandle pipeline) {
  if (pipeline.index < pipelines.pool_size) {
    resource_deletion_queue.push(
        {ResourceDeletionType::Pipeline, pipeline.index, current_frame});
...
void GpuDevice::destroy_sampler(SamplerHandle sampler) {
  if (sampler.index < samplers.pool_size) {
    resource_deletion_queue.push(
        {ResourceDeletionType::Sampler, sampler.index, current_frame});
  } else {
...
void GpuDevice::destroy_descriptor_set_layout(
    DescriptorSetLayoutHandle descriptor_set_layout) {
  if (descriptor_set_layout.index < descriptor_set_layouts.pool_size) {
    resource_deletion_queue.push({ResourceDeletionType::DescriptorSetLayout,
                                  descriptor_set_layout.index, current_frame});
  } else {
...
void GpuDevice::destroy_descriptor_set(DescriptorSetHandle descriptor_set) {
  if (descriptor_set.index < descriptor_sets.pool_size) {
    resource_deletion_queue.push({ResourceDeletionType::DescriptorSet,
                                  descriptor_set.index, current_frame});
  } else {
...
void GpuDevice::destroy_render_pass(RenderPassHandle render_pass) {
  if (render_pass.index < render_passes.pool_size) {
    resource_deletion_queue.push(
        {ResourceDeletionType::RenderPass, render_pass.index, current_frame});
  } else {
...
void GpuDevice::destroy_shader_state(ShaderStateHandle shader) {
  if (shader.index < shaders.pool_size) {
    resource_deletion_queue.push(
        {ResourceDeletionType::ShaderState, shader.index, current_frame});
  } else {
...
void GpuDevice::destroy_buffer_instant(ResourceHandle buffer) {
  ...
  if (v_buffer && v_buffer->parent_buffer.index == k_invalid_buffer.index) {
    vmaDestroyBuffer(vma_allocator, v_buffer->vk_buffer,
                     v_buffer->vma_allocation);
  }
...
void GpuDevice::destroy_texture_instant(ResourceHandle texture) {
  ...
  if (v_texture) {
    ...
    vkDestroyImageView(vulkan_device, v_texture->vk_image_view,
                       vulkan_allocation_callbacks);
    vmaDestroyImage(vma_allocator, v_texture->vk_image,
                    v_texture->vma_allocation);
  }
...
void GpuDevice::destroy_pipeline_instant(ResourceHandle pipeline) {
  ...
  if (v_pipeline) {
    vkDestroyPipeline(vulkan_device, v_pipeline->vk_pipeline,
                      vulkan_allocation_callbacks);
...
void GpuDevice::destroy_sampler_instant(ResourceHandle sampler) {
  ...
  if (v_sampler) {
    vkDestroySampler(vulkan_device, v_sampler->vk_sampler,
                     vulkan_allocation_callbacks);
  }
  samplers.release_resource(sampler);
}

void GpuDevice::destroy_descriptor_set_layout_instant(
    ResourceHandle descriptor_set_layout) {
  DesciptorSetLayout* v_descriptor_set_layout =
      (DesciptorSetLayout*)descriptor_set_layouts.access_resource(
          descriptor_set_layout);

  if (v_descriptor_set_layout) {
    vkDestroyDescriptorSetLayout(
        vulkan_device, v_descriptor_set_layout->vk_descriptor_set_layout,
        vulkan_allocation_callbacks);
...
void GpuDevice::destroy_descriptor_set_instant(ResourceHandle descriptor_set) {
  DesciptorSet* v_descriptor_set =
      (DesciptorSet*)descriptor_sets.access_resource(descriptor_set);
...
void GpuDevice::destroy_render_pass_instant(ResourceHandle render_pass) {
  RenderPass* v_render_pass =
      (RenderPass*)render_passes.access_resource(render_pass);

  if (v_render_pass) {

    if (v_render_pass->num_render_targets)
      vkDestroyFramebuffer(vulkan_device, v_render_pass->vk_frame_buffer,
                           vulkan_allocation_callbacks);
...
void GpuDevice::destroy_shader_state_instant(ResourceHandle shader) {
  ...
  if (v_shader_state) {
    ...
    for (size_t i = 0; i < v_shader_state->active_shaders; i++) {
      vkDestroyShaderModule(vulkan_device,
                            v_shader_state->shader_stage_info[i].module,
                            vulkan_allocation_callbacks);
    }
  }
...
void GpuDevice::set_resource_name(VkObjectType type, u64 handle,
                                  const char* name) {
...
void GpuDevice::create_swapchain() {
  ...
  vkGetPhysicalDeviceSurfaceSupportKHR(
      vulkan_physical_device, vulkan_queue_family, vulkan_window_surface,
      &surface_supported);
  ...
  vkGetPhysicalDeviceSurfaceCapabilitiesKHR(
      vulkan_physical_device, vulkan_window_surface, &surface_capabilities);
  ...
  rprint("Create swapchain %u %u - saved %u %u, min image %u\n",
         swapchain_extent.width, swapchain_extent.height, swapchain_width,
         swapchain_height, surface_capabilities.minImageCount);
  ...
  VkResult result = vkCreateSwapchainKHR(vulkan_device, &swapchain_create_info,
                                         0, &vulkan_swapchain);
  check(result);

  //// Cache swapchain images
  vkGetSwapchainImagesKHR(vulkan_device, vulkan_swapchain,
                          &vulkan_swapchain_image_count, NULL);
  vkGetSwapchainImagesKHR(vulkan_device, vulkan_swapchain,
  ...
  for (size_t iv = 0; iv < vulkan_swapchain_image_count; iv++) {
    ...
    check(vkCreateImageView(vulkan_device, &view_info,
                            vulkan_allocation_callbacks,
                            &vulkan_swapchain_image_views[iv]));
  }
}
...
void GpuDevice::destroy_swapchain() {
  ...
  for (size_t iv = 0; iv < vulkan_swapchain_image_count; iv++) {
    vkDestroyImageView(vulkan_device, vulkan_swapchain_image_views[iv],
                       vulkan_allocation_callbacks);
    vkDestroyFramebuffer(vulkan_device, vulkan_swapchain_framebuffers[iv],
                         vulkan_allocation_callbacks);
  }

  vkDestroySwapchainKHR(vulkan_device, vulkan_swapchain,
                        vulkan_allocation_callbacks);
}

VkRenderPass GpuDevice::get_vulkan_render_pass(const RenderPassOutput& output,
                                               cstring name) {
  ...
  u64 hashed_memory =
      raptor::hash_bytes((void*)&output, sizeof(RenderPassOutput));
...
//
//
static void vulkan_resize_texture(GpuDevice& gpu, Texture* v_texture,
                                  Texture* v_texture_to_delete, u16 width,
                                  u16 height, u16 depth) {
...
void GpuDevice::resize_swapchain() {
  ...
  vulkan_resize_texture(*this, vk_depth_texture, vk_texture_to_delete,
                        swapchain_width, swapchain_height, 1);
  ...
  swapchain_pass_creation.set_type(RenderPassType::Swapchain)
      .set_name("Swapchain");
  vulkan_create_swapchain_pass(*this, swapchain_pass_creation,
                               vk_swapchain_pass);
...
void GpuDevice::update_descriptor_set_instant(
    const DescriptorSetUpdate& update) {

  // Use a dummy descriptor set to delete the vulkan descriptor set handle
  DescriptorSetHandle dummy_delete_descriptor_set_handle = {
      descriptor_sets.obtain_resource()};
  DesciptorSet* dummy_delete_descriptor_set =
  ...
  dummy_delete_descriptor_set->vk_descriptor_set =
      descriptor_set->vk_descriptor_set;
  ...
  vkAllocateDescriptorSets(vulkan_device, &allocInfo,
                           &descriptor_set->vk_descriptor_set);
...
//
//
void GpuDevice::resize_output_textures(RenderPassHandle render_pass, u32 width,
                                       u32 height) {
  ...
  if (vk_render_pass) {
    ...
    for (u32 i = 0; i < rts; ++i) {
      ...
      vk_texture_to_delete->handle = texture_to_delete;
      vulkan_resize_texture(*this, vk_texture, vk_texture_to_delete, new_width,
                            new_height, 1);
    ...
    if (vk_render_pass->output_depth.index != k_invalid_index) {
      ...
      if (vk_texture->width != new_width || vk_texture->height != new_height) {
        ...
        // Update handle so it can be used to update bindless to dummy texture.
        vk_texture_to_delete->handle = texture_to_delete;
        vulkan_resize_texture(*this, vk_texture, vk_texture_to_delete,
    ...
    RenderPass* vk_render_pass_to_destroy =
        access_render_pass(render_pass_to_destroy);

    vk_render_pass_to_destroy->vk_frame_buffer =
        vk_render_pass->vk_frame_buffer;
    ...
    if (vk_render_pass->vk_frame_buffer) {
      vulkan_create_framebuffer(
          *this, vk_render_pass, vk_render_pass->output_textures,
...
void GpuDevice::fill_barrier(RenderPassHandle render_pass,
  ...
  if (vk_render_pass) {
    ...
    for (u32 i = 0; i < rts; ++i) {
      out_barrier.image_barriers[out_barrier.num_image_barriers++].texture =
          vk_render_pass->output_textures[i];
    }

    if (vk_render_pass->output_depth.index != k_invalid_index) {
      out_barrier.image_barriers[out_barrier.num_image_barriers++].texture =
          vk_render_pass->output_depth;
    }
  }
}
...
void GpuDevice::new_frame() {
  ...
  VkFence* render_complete_fence =
      &vulkan_command_buffer_executed_fence[current_frame];

  if (vkGetFenceStatus(vulkan_device, *render_complete_fence) != VK_SUCCESS) {
    vkWaitForFences(vulkan_device, 1, render_complete_fence, VK_TRUE,
                    UINT64_MAX);
  }
  ...
  VkResult result =
      vkAcquireNextImageKHR(vulkan_device, vulkan_swapchain, UINT64_MAX,
                            vulkan_image_acquired_semaphore[current_frame],
                            VK_NULL_HANDLE, &vulkan_image_index);
  ...
  const u32 used_size =
      dynamic_allocated_size - (dynamic_per_frame_size * previous_frame);
  dynamic_max_per_frame_size =
      raptor_max(used_size, dynamic_max_per_frame_size);
...
void GpuDevice::present() {
  ...
  VkSemaphore* render_complete_semaphore =
      &vulkan_render_complete_semaphore[current_frame];
  ...
  for (u32 c = 0; c < num_queued_command_buffers; c++) {
    ...
    if (command_buffer->is_recording && command_buffer->current_render_pass &&
  ...
  if (timestamps_enabled) {
    if (gpu_timestamp_manager->has_valid_queries()) {
      ...
      const u32 query_offset =
          (current_frame * gpu_timestamp_manager->queries_per_frame) * 2;
      const u32 query_count = gpu_timestamp_manager->current_query * 2;
      vkGetQueryPoolResults(
          vulkan_device, vulkan_timestamp_query_pool, query_offset, query_count,
          sizeof(u64) * query_count * 2,
          &gpu_timestamp_manager->timestamps_data[query_offset],
          sizeof(gpu_timestamp_manager->timestamps_data[0]),
          VK_QUERY_RESULT_64_BIT | VK_QUERY_RESULT_WAIT_BIT);

      // Calculate and cache the elapsed time
      for (u32 i = 0; i < gpu_timestamp_manager->current_query; i++) {
        u32 index =
            (current_frame * gpu_timestamp_manager->queries_per_frame) + i;

        GPUTimestamp& timestamp = gpu_timestamp_manager->timestamps[index];

        double start =
            (double)gpu_timestamp_manager->timestamps_data[(index * 2)];
        double end =
            (double)gpu_timestamp_manager->timestamps_data[(index * 2) + 1];
...
void GpuDevice::set_present_mode(PresentMode::Enum mode) {
  ...
  vkGetPhysicalDeviceSurfacePresentModesKHR(
      vulkan_physical_device, vulkan_window_surface, &supported_count,
...
void GpuDevice::link_texture_sampler(TextureHandle texture,
...
CommandBuffer* GpuDevice::get_command_buffer(QueueType::Enum type, bool begin) {
  CommandBuffer* cb =
      command_buffer_ring.get_command_buffer(current_frame, begin);
  ...
  if (gpu_timestamp_reset && begin) {
    ...
    vkCmdResetQueryPool(
        cb->vk_command_buffer, vulkan_timestamp_query_pool,
        current_frame * gpu_timestamp_manager->queries_per_frame * 2,
...
CommandBuffer* GpuDevice::get_instant_command_buffer() {
  CommandBuffer* cb =
      command_buffer_ring.get_command_buffer_instant(current_frame, false);
  return cb;
}

// Resource Description Query ///////////////////////////////////////////////////

void GpuDevice::query_buffer(BufferHandle buffer,
                             BufferDescription& out_description) {
...
void GpuDevice::query_texture(TextureHandle texture,
                              TextureDescription& out_description) {
  if (texture.index != k_invalid_index) {
    ...
    out_description.render_target =
        (texture_data->flags & TextureFlags::RenderTarget_mask) ==
        TextureFlags::RenderTarget_mask;
    out_description.compute_access =
        (texture_data->flags & TextureFlags::Compute_mask) ==
        TextureFlags::Compute_mask;
...
void GpuDevice::query_pipeline(PipelineHandle pipeline,
...
void GpuDevice::query_sampler(SamplerHandle sampler,
                              SamplerDescription& out_description) {
...
void GpuDevice::query_descriptor_set_layout(
    DescriptorSetLayoutHandle descriptor_set_layout,
    DescriptorSetLayoutDescription& out_description) {
  if (descriptor_set_layout.index != k_invalid_index) {
    const DesciptorSetLayout* descriptor_set_layout_data =
        access_descriptor_set_layout(descriptor_set_layout);

    const u32 num_bindings = descriptor_set_layout_data->num_bindings;
    for (size_t i = 0; i < num_bindings; i++) {
      out_description.bindings[i].name =
          descriptor_set_layout_data->bindings[i].name;
      out_description.bindings[i].type =
          descriptor_set_layout_data->bindings[i].type;
    }

    out_description.num_active_bindings =
        descriptor_set_layout_data->num_bindings;
  }
}
...
void GpuDevice::query_descriptor_set(DescriptorSetHandle descriptor_set,
  ...
  if (descriptor_set.index != k_invalid_index) {
    const DesciptorSet* descriptor_set_data =
        access_descriptor_set(descriptor_set);
...
const RenderPassOutput& GpuDevice::get_render_pass_output(
...
void* GpuDevice::map_buffer(const MapBufferParameters& parameters) {
  ...
  if (buffer->parent_buffer.index == dynamic_buffer.index) {
    ...
    return dynamic_allocate(parameters.size == 0 ? buffer->size
                                                 : parameters.size);
  }
...
void GpuDevice::push_gpu_timestamp(CommandBuffer* command_buffer,
                                   const char* name) {
  if (!timestamps_enabled)
    return;

  u32 query_index = gpu_timestamp_manager->push(current_frame, name);
  vkCmdWriteTimestamp(command_buffer->vk_command_buffer,
...
const ShaderState* GpuDevice::access_shader_state(
    ShaderStateHandle shader) const {
...
DesciptorSetLayout* GpuDevice::access_descriptor_set_layout(
    DescriptorSetLayoutHandle descriptor_set_layout) {
  return (DesciptorSetLayout*)descriptor_set_layouts.access_resource(
      descriptor_set_layout.index);
}

const DesciptorSetLayout* GpuDevice::access_descriptor_set_layout(
    DescriptorSetLayoutHandle descriptor_set_layout) const {
  return (const DesciptorSetLayout*)descriptor_set_layouts.access_resource(
      descriptor_set_layout.index);
}

DesciptorSet* GpuDevice::access_descriptor_set(
    DescriptorSetHandle descriptor_set) {
  return (DesciptorSet*)descriptor_sets.access_resource(descriptor_set.index);
}

const DesciptorSet* GpuDevice::access_descriptor_set(
    DescriptorSetHandle descriptor_set) const {
  return (const DesciptorSet*)descriptor_sets.access_resource(
      descriptor_set.index);
}
...
const RenderPass* GpuDevice::access_render_pass(
    RenderPassHandle render_pass) const {
...
void GPUTimestampManager::init(Allocator* allocator_, u16 queries_per_frame_,
                               u16 max_frames) {
  ...
  const sizet allocated_size =
      sizeof(GPUTimestamp) * queries_per_frame * max_frames +
      sizeof(u64) * queries_per_frame * max_frames * k_data_per_query;
  u8* memory = rallocam(allocated_size, allocator);

  timestamps = (GPUTimestamp*)memory;
  // Data is start, end in 2 u64 numbers.
  timestamps_data =
      (u64*)(memory + sizeof(GPUTimestamp) * queries_per_frame * max_frames);
...
u32 GPUTimestampManager::resolve(u32 current_frame,
                                 GPUTimestamp* timestamps_to_fill) {
  raptor::memory_copy(timestamps_to_fill,
                      &timestamps[current_frame * queries_per_frame],
                      sizeof(GPUTimestamp) * current_query);
...
DeviceCreation& DeviceCreation::set_window(u32 width_, u32 height_,
                                           void* handle) {
  width = (u16)width_;
  height = (u16)height_;
  window = handle;
...
DeviceCreation& DeviceCreation::set_linear_allocator(
