...
raptor::FlatHashMap<raptor::ResourceHandle, raptor::ResourceHandle>
...
void ImGuiService::init(void* configuration) {
  ...
  TextureCreation
      texture_creation;  // = { pixels, ( u16 )width, ( u16 )height, 1, 1, 0, TextureFormat::R8G8B8A8_UNORM, TextureType::Texture2D };
  texture_creation
  ...
  if (gpu->bindless_supported) {
    ...
    descriptor_set_layout_creation
        .add_binding(
            {VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER, 1, 1, "LocalConstants"})
        .set_name("RLL_ImGui");
  }
    ...
    for (int cmd_i = 0; cmd_i < cmd_list->CmdBuffer.Size; cmd_i++) {
      ...
      if (pcmd->UserCallback) {
        ...
        if (clip_rect.x < fb_width && clip_rect.y < fb_height &&
          ...
          commands.draw_indexed(raptor::TopologyType::Triangle, pcmd->ElemCount,
                                1, index_buffer_offset + pcmd->IdxOffset,
