...
struct GPUTimestampManager {
  ...
  u32 resolve(
      u32 current_frame,
      GPUTimestamp*
          timestamps_to_fill);  // Returns the total queries for this frame.

  u32 push(u32 current_frame,
           const char* name);  // Returns the timestamp query index.
  ...
  bool current_frame_resolved =
      false;  // Used to query the GPU only once per frame if get_gpu_timestamps is called more than once per frame.
...
struct GpuDevice : public Service {
  ...
  void resize_output_textures(RenderPassHandle render_pass, u32 width,
                              u32 height);
  ...
  void link_texture_sampler(
      ...
      SamplerHandle
          sampler);  // TODO: for now specify a sampler for a texture or use the default one.
  ...
  void queue_command_buffer(
      CommandBuffer*
          command_buffer);  // Queue command buffer that will not be executed until present is called.
