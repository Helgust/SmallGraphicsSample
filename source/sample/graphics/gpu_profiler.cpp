...
void GPUProfiler::init(Allocator* allocator_, u32 max_frames_) {
  ...
  timestamps =
      (GPUTimestamp*)ralloca(sizeof(GPUTimestamp) * max_frames * 32, allocator);
...
void GPUProfiler::imgui_draw() {
...

  {
    ...
    if (selected_frame >= 0) {
      ...
      for (u32 j = 0; j < per_frame_active[selected_frame]; ++j) {
        ...
        sprintf(buf, "(%d)-%s %2.4f", timestamp.depth, timestamp.name,
  ...
  static const char* items[] = {"200ms", "100ms", "66ms", "33ms",
                                "16ms",  "8ms",   "4ms"};
