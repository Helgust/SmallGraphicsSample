...
struct Transform {
  ...
  mat4s calculate_matrix() const {
    ...
    const mat4s local_matrix = glms_mat4_mul(
        glms_mat4_mul(translation_matrix, glms_quat_mat4(rotation)),
        scale_matrix);
    return local_matrix;
  }
};  // struct Transform
...
static u8* get_buffer_data(raptor::glTF::BufferView* buffer_views,
                           u32 buffer_index, raptor::Array<void*>& buffers_data,
                           u32* buffer_size = nullptr,
...
int main(int argc, char** argv) {
  ...
  for (u32 image_index = 0; image_index < scene.images_count; ++image_index) {
    ...
    TextureResource* tr =
        renderer.create_texture(image.uri.data, image.uri.data);
    RASSERT(tr != nullptr);
  ...
  u32 zero_value = 0;
  texture_creation.set_name("dummy_texture")
      .set_size(1, 1, 1)
  ...
  for (u32 sampler_index = 0; sampler_index < scene.samplers_count;
       ++sampler_index) {
    glTF::Sampler& sampler = scene.samplers[sampler_index];

    char* sampler_name =
        resource_name_buffer.append_use_f("sampler_%u", sampler_index);

    SamplerCreation creation;
    creation.min_filter = sampler.min_filter == glTF::Sampler::Filter::LINEAR
                              ? VK_FILTER_LINEAR
                              : VK_FILTER_NEAREST;
  ...
  for (u32 buffer_index = 0; buffer_index < scene.buffers_count;
       ++buffer_index) {
  ...
  for (u32 buffer_index = 0; buffer_index < scene.buffer_views_count;
       ++buffer_index) {
    char* buffer_name = nullptr;
    u32 buffer_size = 0;
    u8* data = get_buffer_data(scene.buffer_views, buffer_index, buffers_data,
                               &buffer_size, &buffer_name);
    ...
    if (buffer_name == nullptr) {
      buffer_name =
          resource_name_buffer.append_use_f("buffer_%u", buffer_index);
    } else {
      // NOTE(marco); some buffers might have the same name, which causes issues in the renderer cache
      buffer_name =
          resource_name_buffer.append_use_f("%s_%u", buffer_name, buffer_index);
    }

    BufferResource* br = renderer.create_buffer(
        flags, ResourceUsageType::Immutable, buffer_size, data, buffer_name);
    RASSERT(br != nullptr);
  ...
  buffer_creation
      .set(VK_BUFFER_USAGE_VERTEX_BUFFER_BIT, ResourceUsageType::Immutable,
           sizeof(vec4s) * 3)
      .set_data(dummy_data)
...

  {
    ...
    char* vert_path =
        path_buffer.append_use_f("%s%s", RAPTOR_SHADER_FOLDER, vert_file);
    FileReadResult vert_code = file_read_text(vert_path, allocator);

    const char* frag_file = "main.frag";
    char* frag_path =
        path_buffer.append_use_f("%s%s", RAPTOR_SHADER_FOLDER, frag_file);
    ...
    cube_rll_creation.add_binding({VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER, 3,
                                   1, "roughnessMetalnessTexture"});
    cube_rll_creation.add_binding({VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER, 4,
                                   1, "roughnessMetalnessTexture"});
    cube_rll_creation.add_binding(
    ...
    while (node_stack.size) {
      ...
      if (node.matrix_count) {
        ...
        if (node.translation_count) {
          ...
          node_translation = vec3s{node.translation[0], node.translation[1],
                                   node.translation[2]};
        }
        ...
        if (node.rotation_count) {
          ...
          node_rotation = glms_quat_init(node.rotation[0], node.rotation[1],
      ...
      for (u32 primitive_index = 0; primitive_index < mesh.primitives_count;
        ...
        glTF::Accessor& indices_accessor =
            scene.accessors[mesh_primitive.indices];
        RASSERT(
            indices_accessor.component_type == glTF::Accessor::UNSIGNED_INT ||
            indices_accessor.component_type == glTF::Accessor::UNSIGNED_SHORT);
        ...
        glTF::BufferView& indices_buffer_view =
            scene.buffer_views[indices_accessor.buffer_view];
        BufferResource& indices_buffer_gpu =
            buffers[indices_accessor.buffer_view];
        ...
        i32 position_accessor_index = gltf_get_attribute_accessor_index(
            mesh_primitive.attributes, mesh_primitive.attribute_count,
            "POSITION");
        i32 tangent_accessor_index = gltf_get_attribute_accessor_index(
            mesh_primitive.attributes, mesh_primitive.attribute_count,
            "TANGENT");
        i32 normal_accessor_index = gltf_get_attribute_accessor_index(
            mesh_primitive.attributes, mesh_primitive.attribute_count,
            "NORMAL");
        i32 texcoord_accessor_index = gltf_get_attribute_accessor_index(
            mesh_primitive.attributes, mesh_primitive.attribute_count,
            "TEXCOORD_0");

        vec3s* position_data = nullptr;
        u32* index_data_32 = (u32*)get_buffer_data(
            scene.buffer_views, indices_accessor.buffer_view, buffers_data);
        u16* index_data_16 = (u16*)index_data_32;
        u32 vertex_count = 0;

        if (position_accessor_index != -1) {
          glTF::Accessor& position_accessor =
              scene.accessors[position_accessor_index];
          glTF::BufferView& position_buffer_view =
              scene.buffer_views[position_accessor.buffer_view];
          BufferResource& position_buffer_gpu =
              buffers[position_accessor.buffer_view];

          vertex_count = position_accessor.count;

          mesh_draw.position_buffer = position_buffer_gpu.handle;
          mesh_draw.position_offset =
              position_accessor.byte_offset == glTF::INVALID_INT_VALUE
                  ? 0
                  : position_accessor.byte_offset;

          position_data = (vec3s*)get_buffer_data(
              scene.buffer_views, position_accessor.buffer_view, buffers_data);
        } else {
        ...
        if (normal_accessor_index != -1) {
          glTF::Accessor& normal_accessor =
              scene.accessors[normal_accessor_index];
          glTF::BufferView& normal_buffer_view =
              scene.buffer_views[normal_accessor.buffer_view];
          BufferResource& normal_buffer_gpu =
              buffers[normal_accessor.buffer_view];

          mesh_draw.normal_buffer = normal_buffer_gpu.handle;
          mesh_draw.normal_offset =
              normal_accessor.byte_offset == glTF::INVALID_INT_VALUE
                  ? 0
          ...
          for (u32 index = 0; index < index_count; index += 3) {
            ...
            u32 i1 =
                indices_accessor.component_type == glTF::Accessor::UNSIGNED_INT
                    ? index_data_32[index + 1]
                    : index_data_16[index + 1];
            u32 i2 =
                indices_accessor.component_type == glTF::Accessor::UNSIGNED_INT
                    ? index_data_32[index + 2]
                    : index_data_16[index + 2];
          ...
          normals_creation
              .set(VK_BUFFER_USAGE_VERTEX_BUFFER_BIT,
                   ResourceUsageType::Immutable,
                   normals_array.size * sizeof(vec3s))
              .set_name("normals")
              .set_data(normals_array.data);
        ...
        if (tangent_accessor_index != -1) {
          glTF::Accessor& tangent_accessor =
              scene.accessors[tangent_accessor_index];
          glTF::BufferView& tangent_buffer_view =
              scene.buffer_views[tangent_accessor.buffer_view];
          BufferResource& tangent_buffer_gpu =
              buffers[tangent_accessor.buffer_view];

          mesh_draw.tangent_buffer = tangent_buffer_gpu.handle;
          mesh_draw.tangent_offset =
              tangent_accessor.byte_offset == glTF::INVALID_INT_VALUE
                  ? 0
                  : tangent_accessor.byte_offset;

          mesh_draw.material_data.flags |=
              MaterialFeatures_TangentVertexAttribute;
        }

        if (texcoord_accessor_index != -1) {
          glTF::Accessor& texcoord_accessor =
              scene.accessors[texcoord_accessor_index];
          glTF::BufferView& texcoord_buffer_view =
              scene.buffer_views[texcoord_accessor.buffer_view];
          BufferResource& texcoord_buffer_gpu =
              buffers[texcoord_accessor.buffer_view];

          mesh_draw.texcoord_buffer = texcoord_buffer_gpu.handle;
          mesh_draw.texcoord_offset =
              texcoord_accessor.byte_offset == glTF::INVALID_INT_VALUE
                  ? 0
                  : texcoord_accessor.byte_offset;

          mesh_draw.material_data.flags |=
              MaterialFeatures_TexcoordVertexAttribute;
        }

        RASSERTM(mesh_primitive.material != glTF::INVALID_INT_VALUE,
                 "Mesh with no material is not supported!");
        ...
        if (material.pbr_metallic_roughness != nullptr) {
          if (material.pbr_metallic_roughness->base_color_factor_count != 0) {
            RASSERT(material.pbr_metallic_roughness->base_color_factor_count ==
                    4);
            ...
            mesh_draw.material_data.base_color_factor = {1.0f, 1.0f, 1.0f,
                                                         1.0f};
          }

          if (material.pbr_metallic_roughness->base_color_texture != nullptr) {
            glTF::Texture& diffuse_texture =
                scene.textures[material.pbr_metallic_roughness
                                   ->base_color_texture->index];
            TextureResource& diffuse_texture_gpu =
                images[diffuse_texture.source];
            ...
            ds_creation.texture_sampler(diffuse_texture_gpu.handle,
                                        sampler_handle, 2);
          ...
          if (material.pbr_metallic_roughness->metallic_roughness_texture !=
            ...
            TextureResource& roughness_texture_gpu =
            ...
            ds_creation.texture_sampler(roughness_texture_gpu.handle,
                                        sampler_handle, 3);
          ...
          if (material.pbr_metallic_roughness->metallic_factor !=
            ...
            mesh_draw.material_data.metallic_factor =
          ...
          if (material.pbr_metallic_roughness->roughness_factor !=
              glTF::INVALID_FLOAT_VALUE) {
            mesh_draw.material_data.roughness_factor =
        ...
        if (material.occlusion_texture != nullptr) {
          glTF::Texture& occlusion_texture =
              scene.textures[material.occlusion_texture->index];

          // NOTE(marco): this could be the same as the roughness texture, but for now we treat it as a separate
          // texture
          TextureResource& occlusion_texture_gpu =
              images[occlusion_texture.source];
          ...
          ds_creation.texture_sampler(occlusion_texture_gpu.handle,
                                      sampler_handle, 4);

          mesh_draw.material_data.occlusion_factor =
              material.occlusion_texture->strength != glTF::INVALID_FLOAT_VALUE
        ...
        if (material.emissive_texture != nullptr) {
          glTF::Texture& emissive_texture =
              scene.textures[material.emissive_texture->index];

          // NOTE(marco): this could be the same as the roughness texture, but for now we treat it as a separate
          // texture
          TextureResource& emissive_texture_gpu =
              images[emissive_texture.source];
          ...
          ds_creation.texture_sampler(emissive_texture_gpu.handle,
                                      sampler_handle, 5);
        ...
        if (material.normal_texture != nullptr) {
          glTF::Texture& normal_texture =
              scene.textures[material.normal_texture->index];
          ...
          ds_creation.texture_sampler(normal_texture_gpu.handle, sampler_handle,
                                      6);
  ...
  while (!window.requested_exit) {
    ...
    mat4s global_model = {};
    {
      ...
      if (cb_data) {
        if (input_handler.is_mouse_down(MouseButtons::MOUSE_BUTTONS_LEFT) &&
          ...
          pitch += (input_handler.mouse_position.y -
                    input_handler.previous_mouse_position.y) *
                   0.1f;
          yaw += (input_handler.mouse_position.x -
                  input_handler.previous_mouse_position.x) *
                 0.3f;
          ...
          mat3s rxm = glms_mat4_pick3(
              glms_rotate_make(glm_rad(-pitch), vec3s{1.0f, 0.0f, 0.0f}));
          mat3s rym = glms_mat4_pick3(
              glms_rotate_make(glm_rad(-yaw), vec3s{0.0f, 1.0f, 0.0f}));
        ...
        mat4s view =
            glms_lookat(eye, glms_vec3_add(eye, look), vec3s{0.0f, 1.0f, 0.0f});
        mat4s projection = glms_perspective(
            glm_rad(60.0f), gpu.swapchain_width * 1.0f / gpu.swapchain_height,
            0.01f, 1000.0f);
        ...
        mat4s sm =
            glms_scale_make(vec3s{model_scale, model_scale, model_scale});
    ...
    if (!window.minimized) {
      raptor::CommandBuffer* gpu_commands =
          gpu.get_command_buffer(QueueType::Graphics, true);
      ...
      for (u32 mesh_index = 0; mesh_index < mesh_draws.size; ++mesh_index) {
        ...
        mesh_draw.material_data.model_inv = glms_mat4_inv(glms_mat4_transpose(
            glms_mat4_mul(global_model, mesh_draw.material_data.model)));

        MapBufferParameters material_map = {mesh_draw.material_buffer, 0, 0};
        MaterialData* material_buffer_data =
            (MaterialData*)gpu.map_buffer(material_map);

        memcpy(material_buffer_data, &mesh_draw.material_data,
               sizeof(MaterialData));

        gpu.unmap_buffer(material_map);

        gpu_commands->bind_vertex_buffer(mesh_draw.position_buffer, 0,
                                         mesh_draw.position_offset);
        gpu_commands->bind_vertex_buffer(mesh_draw.normal_buffer, 2,
                                         mesh_draw.normal_offset);

        if (mesh_draw.material_data.flags &
            MaterialFeatures_TangentVertexAttribute) {
          gpu_commands->bind_vertex_buffer(mesh_draw.tangent_buffer, 1,
                                           mesh_draw.tangent_offset);
        } else {
        ...
        if (mesh_draw.material_data.flags &
            MaterialFeatures_TexcoordVertexAttribute) {
          gpu_commands->bind_vertex_buffer(mesh_draw.texcoord_buffer, 3,
                                           mesh_draw.texcoord_offset);
        } else {
          gpu_commands->bind_vertex_buffer(dummy_attribute_buffer, 3, 0);
        }

        gpu_commands->bind_index_buffer(mesh_draw.index_buffer,
                                        mesh_draw.index_offset,
                                        mesh_draw.index_type);
        gpu_commands->bind_descriptor_set(&mesh_draw.descriptor_set, 1, nullptr,
                                          0);

        gpu_commands->draw_indexed(TopologyType::Triangle, mesh_draw.count, 1,
                                   0, 0, 0);
      }
